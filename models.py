...
class VegetableCategory(db.Model):
    ...
    vegetables: Mapped[List['Vegetable']] = relationship(back_populates='category')
...
class Vegetable(db.Model):
    ...
    category: Mapped[VegetableCategory] = relationship(back_populates='vegetables')

    publisher_id: Mapped[int] = mapped_column(Integer, ForeignKey('user.id'))
    publisher: Mapped[User] = relationship(back_populates='vegetables')
